...
export const checkAppUpdateStatus = async (): Promise<{
  ...
  const currentVersion = localStorage.getItem('coinburst_installed_ver') || '2.18.0';
    ...
    if (res.ok) {
      ...
      if (isNewVer || isNewTime) {
        return {
          ...
          currentVersion,
          latestVersion: data.version || '2.18.0',
          buildTime: data.buildTime,
          reason: `Build v${data.version || '2.18.0'} Code ${data.versionCode || 31}`
        };
      }
    }
  } catch (err) {
        ...
        if (res.ok) {
          ...
          if (data.version && data.buildTime) {
            ...
            if (isNewVer || isNewTime) {
              ...
              setShowModal(true);
              setUpdateReason(`Build v${data.version} (Code ${data.versionCode || 31}) deployed`);
              sendLocalNotification({
                id: 99992,
    ...
    const unsubscribeFirebase = onValue(remoteMetaRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        if (isNewVer || isNewTime) {
          ...
          setShowModal(true);
          setUpdateReason(`Live Broadcast v${remoteVer || '2.18.0'} active`);
        }
      }
    });
